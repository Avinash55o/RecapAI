...
const electronAPI = {
  ...
  // ── Q&A Library ──
  qaGetAll: () => ipcRenderer.invoke("qa:getAll"),
  qaAdd: (entry: any) => ipcRenderer.invoke("qa:add", entry),
  qaUpdate: (id: string, updates: any) => ipcRenderer.invoke("qa:update", id, updates),
  qaDelete: (id: string) => ipcRenderer.invoke("qa:delete", id),
  qaSearch: (query: string) => ipcRenderer.invoke("qa:search", query),

  // ── Audio Listen Toggle ──
  triggerAudioListen: () => ipcRenderer.invoke("audio:toggle"),
  onToggleAudioListen: (callback: () => void) => {
    const subscription = () => callback()
    ipcRenderer.on("toggle-audio-listen", subscription)
    return () => {
      ipcRenderer.removeListener("toggle-audio-listen", subscription)
    }
  },

  // ── Click-through: toggle OS-level ignore mouse events ──
  // When ignore=true, clicks pass through the transparent window to apps below.
  // When ignore=false, the window captures clicks (needed while over the pill/panels).
