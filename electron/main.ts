...
// Register the recap-ai protocol
if (process.platform === "darwin") {
  app.setAsDefaultProtocolClient("recap-ai")
} else {
  app.setAsDefaultProtocolClient("recap-ai", process.execPath, [
...
async function createWindow(): Promise<void> {
  ...
  const windowSettings: Electron.BrowserWindowConstructorOptions = {
    // Full work-area size: lets the pill navbar sit anywhere on screen.
    // The transparent window delegates click-through to CSS pointer-events.
    ...
    movable: false  // Window itself is fixed; pill is dragged via -webkit-app-region
  }
  ...
  // Grant microphone permission for audio listen feature (Ctrl+L)
  ...
  // ── Click-through: start in pass-through mode ──────────────────────────
  // forward: true = mousemove is still forwarded to the renderer so our
  // App.tsx mousemove handler can detect when the cursor enters the pill/panels
  // and call set-ignore-mouse = false to re-enable click capture.
...
function showMainWindow(): void {
  if (!state.mainWindow?.isDestroyed()) {
    ...
    state.mainWindow.setIgnoreMouseEvents(true, { forward: true }); // renderer controls this per-hover
    state.mainWindow.setAlwaysOnTop(true, "screen-saver", 1);
    state.mainWindow.setVisibleOnAllWorkspaces(true, {
      visibleOnFullScreen: true
    });
    ...
    // Use the saved opacity (minimum 0.3 so the window is always visible after Ctrl+B)
    const savedOpacity = configHelper.getOpacity();
    const visibleOpacity = Math.max(0.3, savedOpacity);
    state.mainWindow.setOpacity(visibleOpacity);
    state.isWindowVisible = true;
    console.log(`Window shown, opacity set to ${visibleOpacity} (saved: ${savedOpacity})`);
  }
}
...
function setWindowDimensions(_width: number, _height: number): void {
  // NO-OP: the window is now full work-area sized and fixed.
  // Previously this called setBounds() which caused a shaking feedback loop:
  //   renderer resize event → updateContentDimensions IPC → setBounds()
  //   → Electron resize event → renderer resize event → … (infinite loop)
  // CSS pointer-events:none on #root handles click-through instead.
}
...
async function initializeApp() {
  try {
    ...
    const appDataPath = path.join(app.getPath('appData'), 'RecapAI')
