...
interface Config {
  ...
  /** 'groq' = Groq Whisper Large (fast cloud, needs Groq key)
   *  'openai' = OpenAI Whisper-1 (cloud, needs OpenAI key)
   *  'local' = Whisper Base WASM (offline, no key) */
  transcriptionEngine: 'groq' | 'openai' | 'local';
  /** Groq API key for Whisper Large v3 transcription */
  groqApiKey: string;
}
...
export class ConfigHelper extends EventEmitter {
  ...
  private defaultConfig: Config = {
    ...
    extractionModel: "gemini-3.1-flash-lite-preview", // Default to Flash for faster responses
    solutionModel: "gemini-3.1-flash-lite-preview",
    debuggingModel: "gemini-3.1-flash-lite-preview",
    language: "python",
    opacity: 1.0,
    // RecallAI additions
    personalPrompt: "",
    outputLanguage: "english",
    meetingAudioLanguage: "en",
    transcriptionEngine: "groq",  // Default to Groq (fastest + most accurate)
    groqApiKey: "",
  };
  ...
  private sanitizeModelSelection(model: string, provider: "openai" | "gemini" | "anthropic"): string {
    if (provider === "openai") {
      ...
      const allowedModels = ['gemini-1.5-pro', 'gemini-3.1-flash-lite-preview', 'gemini-2.0-flash', 'gemini-2.0-flash-lite'];
      if (!allowedModels.includes(model)) {
        console.warn(`Invalid Gemini model specified: ${model}. Using default model: gemini-3.1-flash-lite-preview`);
        return 'gemini-3.1-flash-lite-preview'; // safe default
      }
      return model;
  ...
  public loadConfig(): Config {
    try {
      if (fs.existsSync(this.configPath)) {
        ...
        // ── Startup migration: fix stale Gemini model names ───────────────
        // If the saved config has old/wrong model names, upgrade them automatically
        const staleGeminiModels = ['gemini-2.0-flash', 'gemini-3.1-flash-lite', 'gemini-1.5-flash'];
        const correctGeminiModel = 'gemini-3.1-flash-lite-preview';
        let needsSave = false;
        if (config.apiProvider === 'gemini') {
          if (staleGeminiModels.includes(config.extractionModel)) {
            config.extractionModel = correctGeminiModel; needsSave = true;
          }
          if (staleGeminiModels.includes(config.solutionModel)) {
            config.solutionModel = correctGeminiModel; needsSave = true;
          }
          if (staleGeminiModels.includes(config.debuggingModel)) {
            config.debuggingModel = correctGeminiModel; needsSave = true;
          }
        }

        const merged = { ...this.defaultConfig, ...config };
        if (needsSave) this.saveConfig(merged);  // write corrections back to disk
        return merged;
      }
  ...
  public updateConfig(updates: Partial<Config>): Config {
    try {
      ...
      // Explicit provider from UI always wins — do NOT auto-override it
      // Only auto-detect if the user did NOT explicitly select a provider
      ...
      if (updates.apiProvider && updates.apiProvider !== currentConfig.apiProvider) {
        if (updates.apiProvider === "openai") {
          ...
          updates.extractionModel = "gemini-3.1-flash-lite-preview";
          updates.solutionModel = "gemini-3.1-flash-lite-preview";
          updates.debuggingModel = "gemini-3.1-flash-lite-preview";
        }
      }
