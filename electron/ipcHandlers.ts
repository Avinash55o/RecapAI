...
import { qaLibraryHelper } from "./QALibraryHelper"
...
export function initializeIpcHandlers(deps: IIpcHandlerDeps): void {
  ...
  // Window dimension handlers — no-op now (window is full-screen fixed)
  ipcMain.handle(
    "update-content-dimensions",
    async (_event, _dims: { width: number; height: number }) => { /* no-op */ }
  )

  ipcMain.handle(
    "set-window-dimensions",
    (_event, _width: number, _height: number) => { /* no-op */ }
  )

  // ── Click-through: toggle OS-level mouse event pass-through ──────────────
  // ignore=true  → clicks fall through to whatever is behind the window (other apps)
  // ignore=false → window captures clicks normally (used when hovering pill/panels)
  ...
  // ── Q&A Library handlers ──────────────────────────────────────────────────

  ipcMain.handle("qa:getAll", () => {
    return qaLibraryHelper.loadQALibrary();
  })

  ipcMain.handle("qa:add", (_event, entry) => {
    return qaLibraryHelper.saveQAEntry(entry);
  })

  ipcMain.handle("qa:update", (_event, id: string, updates) => {
    return qaLibraryHelper.updateQAEntry(id, updates);
  })

  ipcMain.handle("qa:delete", (_event, id: string) => {
    return qaLibraryHelper.deleteQAEntry(id);
  })

  ipcMain.handle("qa:search", (_event, query: string) => {
    return qaLibraryHelper.fuzzySearch(query);
  })

  // ── Audio listen toggle ───────────────────────────────────────────────────
  // (triggered by Ctrl+L shortcut; notifies renderer to start/stop mic)
  ipcMain.handle("audio:toggle", () => {
    const mainWindow = deps.getMainWindow()
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send("toggle-audio-listen")
    }
    return { success: true }
  })
}
